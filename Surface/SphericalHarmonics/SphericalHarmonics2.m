function [x,y,z,r] = SphericalHarmonics2(shDegree,shOrder,shRef,shScale...
    ,shType,meshSize)
...
% shRef:radius of reference sphere. If this value is > 0 then spherical
%       harmonics output will be added to a sphere with this much radius.
%       Keep it in the range of 0 to 1
% shScale:scaling applied to spherical harmonics. Lower vlaue will give
%         smooth sphere. keep it above zero good choice is from 0.01 to
%         1
...
switch shType
    case 'real'
        r = abs(shRef+shScale*real(SH/SHmax));
    case 'imag'
        r = abs(shRef+shScale*imag(SH/SHmax));
    case 'abs'
        r = abs(shRef+shScale*abs(SH/SHmax));
end
