...
rot=theta;
for n=1:numPoly
    patch(len*x2,len*y2+2,'r','facecolor',colormap(n,:)...
        ,'edgecolor',colormap(n,:));
    patch(-len*x2,-len*y2,'r','facecolor',colormap(n,:)...
        ,'edgecolor',colormap(n,:));
    patch(-len*x2+0.866,-len*y2+1.5,'r','facecolor',colormap(n,:)...
        ,'edgecolor',colormap(n,:));    
    patch(len*x2-0.866,len*y2+0.5,'r','facecolor',colormap(n,:)...
        ,'edgecolor',colormap(n,:));
    patch(len*x2+0.866,len*y2+0.5,'r','facecolor',colormap(n,:)...
        ,'edgecolor',colormap(n,:));
    patch(-len*x2-0.866,-len*y2+1.5,'r','facecolor',colormap(n,:)...
